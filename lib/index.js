import { ReadOnlyMap, ReadOnlySet, every } from './utils.js';
...
const buildTerminals = (productions, grammar) => {
  const { aliases } = grammar;
  const terminals = new Map();
  // const nodeTypes = aliases.get('Node');

  for (const [type, production] of productions) {
    const concreteTypes = aliases.get(type);
    if (concreteTypes) {
      // type is an alias
      ...
      // if (!nodeTypes.has(type)) throw new Error(`{type: ${type}} is not a known node type`);

      const concreteType = type;
      const nextProduction = terminals.get(concreteType);

      terminals.set(concreteType, function bindNext(props) {
...
const buildAliases = (rawAliases) => {
  ...
  for (const alias of rawAliases) {
    ...
    aliases.set(alias[0], ReadOnlySet.from(alias[1]));
  }

  for (const types of aliases.values()) {
    explodeSubtypes(aliases, new Set(types), types);
  }
...
export class Grammar {
  constructor(grammar) {
    const { base, context, productions, aliases: rawAliases = empty } = grammar;
    const aliases = buildAliases(rawAliases, productions);

    this[_] = { base, context, aliases };
    this[_].terminals = buildTerminals(productions, this);
  }
