...
const buildTerminals = (productions, enhancers, grammar) => {
  ...
  for (let [type, production] of productions) {
    if (aliases.has(type)) {
      throw new Error('type must not be be an alias and a terminal');
    }

    [type, production] = compose(...enhancers)([type, production]);

    const nextProduction = terminals.get(type);

    terminals.set(type, function bindNext(props) {
