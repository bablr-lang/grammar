import { ReadOnlyMap, every, compose } from './utils.js';
...
export const productionFor = (type, value, annotations) => ({ type, value, annotations });

const buildTerminals = (productions, grammar) => {
  const terminals = new Map();

  for (const production of productions) {
    const { type, value } = production;
    ...
    terminals.set(type, {
      ...production,
      value(props) {
        return value(props, grammar, baseProduction);
      },
    });
  }
...
export class Grammar {
  constructor(grammar, enhancers = empty) {
    const {
      base,
      context,
      productions,
      aliases: rawAliases = empty,
    } = compose(...enhancers)(grammar);
