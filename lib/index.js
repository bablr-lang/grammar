...
const buildProductionByType = (productions, concreteTypesByType) => {
  ...
  for (const [type, production] of productions) {
    ...
    if (concreteTypes) {
      ...
      productionByType.set(type, function bindNext(props) {
        return production(props, nextProduction);
      });
    }
  }
...
export class Grammar {
  constructor(grammar, base) {
    ...
    this.productionByType = buildProductionByType(productions, this.concreteTypesByType);
  }
